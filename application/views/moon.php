<div></div>
<script type="text/javascript" src="/js/EventHelpers.js"></script>
<script type="text/javascript" src="/js/cssQuery-p.js"></script>
<script type="text/javascript" src="/js/jcoglan.com/sylvester.js"></script>
<script type="text/javascript" src="/js/cssSandpaper.js"></script>
<script type="text/javascript" src="/js/less-1.6.2.min.js"></script>
<script type="application/javascript">
</script>
<div class="moon-scene visible-desktop visible-tablet">
    <div class="the-moon">
        <div class="reward-banner">
            <h2 class="reward-banner-text">REWARD</h2>
            <div class="no-size"><div class="spaceship spaceship-left"></div></div>
            <div class="no-size"><div class="spaceship spaceship-right"></div></div>
            <h2 class="reward-banner-text">MONIES</h2>
        </div>

    </div>
    <div class="the-earth"></div>
</div>
<?php

?>