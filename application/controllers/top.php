...
class Top extends CI_Controller {
    ...
    private function _getTopTenTableAll()
    {
        ...
        $response = $this->db->query('SELECT users.nickname AS `nickname`, SUM(transactions.shibetoshi) / 100000000 AS `doge`
                                      FROM users
    ...
    private function _getTopTenTableCharity($id)
    {
        if(!empty($id))
        {
            ...
            $response = $this->db->query(
                'SELECT users.nickname AS `nickname`, SUM(transactions.shibetoshi) / 100000000 AS `doge`
                         FROM users
                         JOIN transactions ON transactions.shibetoshi
                         WHERE transactions.user_id = users.id AND transactions.charity_id = ?
                         GROUP BY `nickname`
                         ORDER BY `doge` DESC
                         LIMIT 10', $id
            );
